...
public class VFXBuilders {
	...
	public static class ScreenVFXBuilder {
		...
		VertexFormat format;
		Supplier<ShaderProgram> shader = GameRenderer::getPositionTexProgram;
		Identifier texture;
		...
		public ScreenVFXBuilder end() {
			...
			BufferRenderer.draw(renderedBuffer);
			return this;
		}
	...
	public static class WorldVFXBuilder {
		...
		public WorldVFXBuilder renderTrail(VertexConsumer vertexConsumer, MatrixStack stack, Vector3f offset, List<Vector4f> trailSegments, Function<Float, Float> widthFunc) {
			return renderTrail(vertexConsumer, stack, offset, trailSegments, widthFunc, f -> {

			});
		}

		public WorldVFXBuilder renderTrail(VertexConsumer vertexConsumer, MatrixStack stack, Vector3f offset, List<Vector4f> trailSegments, Function<Float, Float> widthFunc, Consumer<Float> vfxOperator) {
			return renderTrail(vertexConsumer, stack.peek().getPositionMatrix(), offset, trailSegments, widthFunc, vfxOperator);
		}
